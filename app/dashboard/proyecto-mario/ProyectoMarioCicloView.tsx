...
import {
  proyectoMarioCiclosService, PROYECTO_MARIO_CICLO_CONFIG,
  type ProyectoMarioCicloTipo, type ProyectoMarioCicloCompleto, type ProyectoMarioCicloParticipante, type ProyectoMarioCicloFecha,
} from "@/lib/mod/proyecto-mario-ciclos-service"
...
interface ProyectoMarioCicloViewProps {
  tipo: ProyectoMarioCicloTipo
  canEdit: boolean
}

export function ProyectoMarioCicloView({ tipo, canEdit }: ProyectoMarioCicloViewProps) {
  const router = useRouter()
  const { user } = useAuth()
  const { checkAndExecute } = useSecurityCheck()
  const config = PROYECTO_MARIO_CICLO_CONFIG[tipo]

  const [data, setData] = useState<ProyectoMarioCicloCompleto | null>(null)
  ...
  const [editingParticipant, setEditingParticipant] = useState<ProyectoMarioCicloParticipante | null>(null)
  ...
  const loadData = useCallback(async () => {
    try {
      const result = await proyectoMarioCiclosService.getCicloActivoCompleto(tipo)
      setData(result)
    } catch (error) {
  ...
  useRealtimeMultiple(
    ["proyecto_mario_ciclos", "proyecto_mario_ciclo_participantes", "proyecto_mario_ciclo_fechas", "proyecto_mario_ciclo_asistencia"],
    () => loadData()
  )

  const cicloTerminado = data ? proyectoMarioCiclosService.cicloTerminado(data.fechas) : false

  const handleIniciarCiclo = async () => {
    if (!fechaInicio) return
    setSaving(true)
    try {
      await proyectoMarioCiclosService.iniciarCiclo(tipo, fechaInicio, audit)
  ...
  const handleAddParticipant = async () => {
    ...
    setSaving(true)
    try {
      await proyectoMarioCiclosService.addParticipante(data.ciclo.id, newParticipantName.trim(), audit)
  ...
  const handleEditParticipant = async () => {
    ...
    setSaving(true)
    try {
      await proyectoMarioCiclosService.updateParticipante(editingParticipant.id, editingParticipant.nombre.trim(), audit)
  ...
  const handleDeleteParticipant = async (id: number) => {
    try {
      await proyectoMarioCiclosService.deleteParticipante(id, audit)
      await loadData()
    } catch (error: any) {
      toast.error("Error: " + error.message)
    }
  ...
  const handleEstatusChange = async (participanteId: number, estatus: string) => {
    try {
      await proyectoMarioCiclosService.updateEstatus(participanteId, estatus as "en_curso" | "aprobado" | "reprobado", audit)
      await loadData()
    } catch (error: any) {
      toast.error("Error: " + error.message)
    }
  ...
  const handleAttendanceChange = async (participanteId: number, fechaId: number, status: string) => {
    if (!data) return
    try {
      await proyectoMarioCiclosService.upsertAsistencia(data.ciclo.id, participanteId, fechaId, status as "A" | "J" | "F" | "AT" | "none")
      await loadData()
    } catch (error: any) {
      toast.error("Error: " + error.message)
    }
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center space-y-4">
            ...
            <p className="text-gray-500">{config.label} tiene {config.totalClases} clases (sesiones semanales)</p>
            {canEdit && (
              <Dialog open={showIniciar} onOpenChange={setShowIniciar}>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>Seleccione la fecha de inicio. Se calcularán {config.totalClases} sesiones semanales consecutivas.</DialogDescription>
                  </DialogHeader>
                  <div>
                    <Label>Fecha de inicio</Label>
                    <Input type="date" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)} />
                    <p className="text-xs text-gray-500 mt-1">Las clases serán semanales a partir de esta fecha.</p>
                  </div>
                  <DialogFooter>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-2">
              ...
              {!cicloTerminado && <Badge className="bg-purple-100 text-purple-800">En curso</Badge>}
              ...
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    ...
                    onClick={async () => {
                      try {
                        await proyectoMarioCiclosService.cerrarCiclo(data.ciclo.id, audit)
          ...
          {data.participantes.some((p) => p.estatus === "aprobado") && (
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  await downloadCertificadosProyectoMario(aprobados, tipo)
              ...
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    ...
                    onClick={async () => {
                      try {
                        await proyectoMarioCiclosService.deleteAllFechas(data.ciclo.id, audit)
