...
export function NominaSection() {
  ...
  const [formData, setFormData] = useState({
    cedula: "", nombre: "", telefono: "", email: "", valor: "",
    descuento: "" as string, detalle: "",
  ...
  const resetForm = () => {
    setFormData({ cedula: "", nombre: "", telefono: "", email: "", valor: "",
      descuento: "", detalle: "",
      primera_quincena_pagada: false, primera_quincena_valor: "", primera_quincena_fecha: "", primera_quincena_metodo: "Banco",
      segunda_quincena_pagada: false, segunda_quincena_valor: "", segunda_quincena_fecha: "", segunda_quincena_metodo: "Banco",
    })
  ...
  const registerEgreso = async (nombre: string, valor: number, fecha: string, metodo: string, quincena: string, detalle: string | null) => {
    ...
    try {
      await storage.addEgreso(currentMonth.id, {
        ...
        categoria_principal: "Pago de nómina",
  ...
  const sendPaymentNotification = async (nombre: string, telefono: string | null, email: string | null, valor: number, metodo: string, quincena: "primera" | "segunda") => {
    ...
    if (telefono) {
      const msg = esBanco ? [
        `💰 *Pago de Nómina — Iglesia Regalo de Dios*`, ``,
        `Hola *${nombre}*,`, ``,
        `Tu *${label}* ha sido depositada exitosamente.`, ``,
        `💵 *Valor:* $${valor.toFixed(2)}`,
        `🏦 *Método:* Transferencia bancaria`, ``,
        `Por favor revisa tu cuenta bancaria para confirmar.`, ``,
        `¡Dios te bendiga! 🙏`, `— Administración IRDD`,
      ].join("\n") : [
        `💰 *Pago de Nómina — Iglesia Regalo de Dios*`, ``,
        `Hola *${nombre}*,`, ``,
        `Tu *${label}* ha sido cancelada en efectivo.`, ``,
        `💵 *Valor:* $${valor.toFixed(2)}`,
        `💳 *Método:* Efectivo`, ``,
        `Queda registrado el pago. ¡Dios te bendiga! 🙏`, `— Administración IRDD`,
      ].join("\n")
    ...
    if (email) {
      const contenido = esBanco
        ? `<p>Tu <strong>${label}</strong> ha sido depositada exitosamente en tu cuenta bancaria.</p><p>Por favor revisa tu estado de cuenta para confirmar la recepción.</p>`
        : `<p>Tu <strong>${label}</strong> ha sido cancelada en efectivo.</p><p>Queda registrado el pago correctamente.</p>`
      fetch("/api/send-email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        to: email, subject: `💰 Tu ${label} ha sido cancelada — IRDD`,
        html: `<div style="font-family:'Segoe UI',sans-serif;max-width:600px;margin:0 auto;background:#f3f4f6;padding:32px 16px;">
          <div style="background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            <div style="background:#059669;padding:28px 32px;text-align:center;">
              <div style="font-size:36px;margin-bottom:8px;">💰</div>
              <h1 style="color:white;margin:0;font-size:22px;">Pago de Nómina</h1>
            </div>
            <div style="padding:32px;">
              ...
              <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:20px;margin:20px 0;text-align:center;">
                <p style="color:#6b7280;font-size:12px;margin:0;">VALOR</p>
                <p style="font-size:28px;font-weight:700;color:#059669;margin:8px 0;">$${valor.toFixed(2)}</p>
                <p style="color:#6b7280;font-size:13px;margin:0;">${quincena === "primera" ? "Primera" : "Segunda"} Quincena · ${metodo}</p>
              </div>
              <p style="font-size:14px;color:#374151;">¡Dios te bendiga!</p>
              <p style="color:#9ca3af;font-size:12px;text-align:center;margin-top:24px;">Administración — Iglesia Regalo de Dios</p>
            </div>
          </div>
        </div>`,
      }) }).catch(() => {})
  ...
  const handleAdd = async () => {
    if (!formData.cedula || !formData.nombre || !formData.valor || !currentMonth) { toast.error("Complete los campos obligatorios"); return }
    setSaving(true)
    try {
      const { error } = await supabase.from("nomina").insert({
        ...
        valor: parseFloat(formData.valor), descuento: formData.descuento || null,
        categoria_principal: "Pago de nómina", detalle: formData.detalle || null,
        primera_quincena_pagada: formData.primera_quincena_pagada,
        primera_quincena_valor: formData.primera_quincena_pagada ? parseFloat(formData.primera_quincena_valor) || 0 : null,
        ...
        segunda_quincena_valor: formData.segunda_quincena_pagada ? parseFloat(formData.segunda_quincena_valor) || 0 : null,
      ...
      setShowAddModal(false); resetForm(); loadNomina(true)
  ...
  const handleEdit = (record: NominaRecord) => {
    ...
    setFormData({
      cedula: record.cedula, nombre: record.nombre,
      telefono: record.telefono || "", email: record.email || "",
      valor: record.valor.toString(), descuento: record.descuento || "", detalle: record.detalle || "",
      ...
      primera_quincena_metodo: record.primera_quincena_metodo || "Banco",
      ...
      segunda_quincena_metodo: record.segunda_quincena_metodo || "Banco",
    })
    setShowEditModal(true)
  }

  const handleUpdate = async () => {
    if (!editingRecord || !formData.cedula || !formData.nombre || !formData.valor) { toast.error("Complete los campos obligatorios"); return }
    setSaving(true)
    try {
      const { error } = await supabase.from("nomina").update({
        cedula: formData.cedula, nombre: formData.nombre,
        telefono: formData.telefono || null, email: formData.email || null,
        valor: parseFloat(formData.valor), descuento: formData.descuento || null,
        categoria_principal: "Pago de nómina", detalle: formData.detalle || null,
        primera_quincena_pagada: formData.primera_quincena_pagada,
      ...
      // Notificar y registrar egreso solo si se marcó como pagada AHORA
      ...
      setShowEditModal(false); setEditingRecord(null); resetForm(); loadNomina(true)
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (error) throw error
      toast.success("Registro eliminado"); loadNomina(true)
  ...
  const renderForm = () => (
    <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
      ...
      <div><Label>Valor Total *</Label><Input type="number" value={formData.valor} onChange={(e) => setFormData({ ...formData, valor: e.target.value })} placeholder="0.00" /></div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Select value={formData.detalle} onValueChange={(v) => setFormData({ ...formData, detalle: v })}>
            ...
            <SelectContent>{detalles.map((d) => (<SelectItem key={d} value={d}>{d}</SelectItem>))}</SelectContent>
          </Select>
        </div>
      </div>
  ...
  return (
    <div className="mt-8">
      <Card className="border-amber-200">
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
            <div className="text-center p-3 bg-amber-50 rounded-lg border border-amber-100"><p className="text-xs text-amber-600">Total Nómina</p><p className="text-lg font-bold text-amber-800">${totalNomina.toLocaleString("es-CO", { minimumFractionDigits: 2 })}</p></div>
      ...
      <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
        <DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Agregar a Nómina</DialogTitle><DialogDescription>Registre un nuevo pago</DialogDescription></DialogHeader>
          {renderForm()}
          <DialogFooter><Button variant="outline" onClick={() => setShowAddModal(false)}>Cancelar</Button><Button onClick={handleAdd} disabled={saving}>{saving ? "Guardando..." : "Agregar"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Editar Nómina</DialogTitle><DialogDescription>Modifique los datos</DialogDescription></DialogHeader>
          {renderForm()}
          <DialogFooter><Button variant="outline" onClick={() => { setShowEditModal(false); setEditingRecord(null) }}>Cancelar</Button><Button onClick={handleUpdate} disabled={saving}>{saving ? "Guardando..." : "Guardar"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={showDetallesConfig} onOpenChange={setShowDetallesConfig}>
        <DialogContent>
          <DialogHeader><DialogTitle>Configurar Detalles</DialogTitle><DialogDescription>Opciones del campo Detalle</DialogDescription></DialogHeader>
          <div className="space-y-3">
            <div className="flex gap-2"><Input value={newDetalle} onChange={(e) => setNewDetalle(e.target.value)} placeholder="Nuevo detalle..." /><Button size="sm" onClick={() => { if (newDetalle.trim() && !detalles.includes(newDetalle.trim())) { saveDetalles([...detalles, newDetalle.trim()]); setNewDetalle("") } }}><Plus className="w-4 h-4" /></Button></div>
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {detalles.map((d) => (<div key={d} className="flex items-center justify-between p-2 bg-gray-50 rounded text-sm"><span>{d}</span><button onClick={() => saveDetalles(detalles.filter((x) => x !== d))} className="text-red-500 hover:text-red-700">×</button></div>))}
              {detalles.length === 0 && <p className="text-sm text-gray-400 text-center py-4">No hay detalles configurados</p>}
            </div>
          </div>
