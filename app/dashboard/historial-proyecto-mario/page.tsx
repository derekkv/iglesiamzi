...
import {
  ...
  type ProyectoMarioCicloTipo, type ProyectoMarioCiclo, type ProyectoMarioCicloCompleto,
...
function HistorialContent({ canEdit }: { canEdit: boolean }) {
  ...
  const [expandedCiclo, setExpandedCiclo] = useState<ProyectoMarioCicloCompleto | null>(null)
  ...
  const handleExpandCiclo = async (cicloId: number) => {
    ...
    try {
      const data = await proyectoMarioCiclosService.getCicloCompleto(cicloId)
  ...
  const handleDeleteCiclo = async (cicloId: number) => {
    try {
      ...
      await proyectoMarioCiclosService.deleteAllFechas(cicloId, audit)
      // Eliminar participantes
      const participantes = await proyectoMarioCiclosService.getParticipantes(cicloId)
      for (const p of participantes) {
        await proyectoMarioCiclosService.deleteParticipante(p.id)
      }
  ...
  const handleGenerarCertificados = async (tipo: ProyectoMarioCicloTipo) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-lg sm:text-xl font-semibold text-gray-900">Historial de Proyecto Mario</h1>
            </div>
            {!canEdit && (
              ...
              {ciclos[tab.value].length === 0 ? (
                ...
                <div className="space-y-3">
                  {ciclos[tab.value].map((ciclo) => (
                    <Card key={ciclo.id} className={`transition-all ${expandedCiclo?.ciclo.id === ciclo.id ? "ring-2 ring-purple-300" : ""}`}>
                      <CardContent className="py-4">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          <div className="flex items-center gap-3 cursor-pointer" onClick={() => handleExpandCiclo(ciclo.id)}>
                            <div>
                              <p className="font-medium text-gray-900">
                                {PROYECTO_MARIO_CICLO_CONFIG[tab.value].label}
                              </p>
...
export default function HistorialProyectoMarioPage() {
  return (
    <PermissionsGuard moduleName="historial_proyecto_mario">
