...
import { Trash2, Plus, ArrowLeft, Edit, Lock, Play, CalendarDays, Search } from "lucide-react"
...
export function HerederosCicloView({ tipo, canEdit }: HerederosCicloViewProps) {
  ...
  const [filterNuevo, setFilterNuevo] = useState<"todos" | "si" | "no">("todos")
  ...
  const filteredParticipantes = data ? data.participantes.filter((p) => {
    // Filtro de búsqueda
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase()
      const matchesSearch = p.nombre.toLowerCase().includes(q) ||
        (p.nombre_representante || "").toLowerCase().includes(q) ||
        (p.celular || "").toLowerCase().includes(q) ||
        (p.alergias || "").toLowerCase().includes(q)
      if (!matchesSearch) return false
    }
    // Filtro por nuevo
    if (filterNuevo === "si" && !p.nuevo) return false
    if (filterNuevo === "no" && p.nuevo) return false
    return true
  }) : []
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="w-full px-4 sm:px-6 lg:px-8 py-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select value={filterNuevo} onValueChange={(v) => setFilterNuevo(v as "todos" | "si" | "no")}>
            <SelectTrigger className="h-8 w-28 text-xs">
              <SelectValue placeholder="Nuevo" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="todos">Todos</SelectItem>
              <SelectItem value="si">Nuevos</SelectItem>
              <SelectItem value="no">Regulares</SelectItem>
            </SelectContent>
          </Select>
        ...
        {data.participantes.length > 0 && filteredParticipantes.length === 0 && (
          <p className="text-center py-6 text-gray-500 text-sm">No se encontraron resultados con los filtros aplicados.</p>
        )}
