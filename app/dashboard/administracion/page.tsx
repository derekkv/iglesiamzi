...
import { Pencil, Trash2, RefreshCw, Copy, Check, ArrowLeft, KeyRound } from "lucide-react"
...
function AdministracionContent({ canEdit, canAdmin }: { canEdit: boolean; canAdmin?: boolean }) {
  ...
  const handleResetPassword = async () => {
    if (!canEdit) {
      toast.error("No tiene permisos para resetear contraseñas")
      return
    }
    ...
    const result = await changePassword(resetPasswordUser.id, newPassword)

    setIsResettingPassword(false)

    if (result.success) {
      toast.success(`Contraseña reseteada para ${resetPasswordUser.username}`)
      auditService.log({
        user_id: currentUser!.id,
        user_name: currentUser!.username,
        module: "administracion",
        action: "editar",
        description: `Contraseña reseteada: ${resetPasswordUser.username}`,
        details: { userId: resetPasswordUser.id, username: resetPasswordUser.username },
      })
          ...
          <Dialog open={isResetPasswordDialogOpen} onOpenChange={setIsResetPasswordDialogOpen}>
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Establezca una nueva contraseña para <span className="font-semibold">{resetPasswordUser?.displayName}</span> ({resetPasswordUser?.username})
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                  <p className="text-sm text-amber-800">
                    <strong>Atención:</strong> Esta acción reemplazará la contraseña actual del usuario. Asegúrese de comunicarle la nueva contraseña.
                  </p>
                </div>
                <div className="grid gap-2">
