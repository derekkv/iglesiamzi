...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      action, table, select: selectFields, data, filters, order,
      limit, offset, count, head, single, maybeSingle, onConflict,
    } = body
    ...
    switch (action) {
      case "select": {
        let query = supabase.from(table).select(
          selectFields || "*",
          count ? { count, head: head === true } : undefined
        )
        if (filters) {
        ...
        // Paginación real: si viene offset se usa range(), si no, limit() simple
        if (offset && offset > 0) {
          const to = limit && limit > 0 ? offset + limit - 1 : offset + 999
