...
// Llama a sendSmart() directamente (mismo proceso, sin salto HTTP): usa texto
// libre si la ventana de 24 h está abierta y la plantilla del caso de uso si no.
async function sendWhatsApp(
  phone: string,
  message: string,
  useCase: "recordatorio_5dias" | "recordatorio_manana",
  ...
  try {
    ...
    if (!result.success) {
      console.warn(`[cron-reminders] WhatsApp a ${phone} falló: ${result.error}`)
    }
    return result.success
  } catch (e: any) {
    console.warn("[cron-reminders] Error enviando WhatsApp:", e?.message)
    return false
  }
}
...
async function processAlerts(
  ...
  for (const service of services) {
    ...
    if (userData?.phone) {
      ...
      waOk = await sendWhatsApp(userData.phone, waMessage, alertTypeToUseCase(alertType), {
        userName: service.user_name,
        asignacion: service.asignacion,
        fecha: fechaDisplay,
        horaEntrada: service.hora_entrada || "",
