...
export const censoService = {
  ...
  // ministerios se obtiene de module_groups.display_name (fuente única de verdad)
  async getConfiguraciones(): Promise<ConfiguracionesGlobales | null> {
    const [configResult, ministeriosResult] = await Promise.all([
      supabase.from("configuraciones_globales").select("*").limit(1).single(),
      supabase.from("module_groups").select("display_name").order("sort_order", { ascending: true }),
    ])

    if (configResult.error) throw configResult.error
    const config = configResult.data
    // Sobreescribir ministerios con los de module_groups
    if (ministeriosResult.data) {
      config.ministerios = ministeriosResult.data.map((g: any) => g.display_name)
    }
    return config
  },
