...
class QueryBuilder implements PromiseLike<QueryResult> {
  ...
  select(fields: string = "*", options?: { count?: string; head?: boolean }): QueryBuilder {
    ...
    if (options?.count) this.state.countMode = options.count as "exact" | "planned" | "estimated"
  ...
  /** Paginación real: traduce a offset + limit en el servidor. */
  range(from: number, to: number): QueryBuilder {
    this.state.offsetCount = from
  ...
  private async execute(): Promise<QueryResult> {
    ...
    try {
      ...
      if (this.state.offsetCount) body.offset = this.state.offsetCount
      if (this.state.countMode) body.count = this.state.countMode
      if (this.state.headOnly) body.head = true
